...
export default function SpotifyDashboard() {
  ...
  const [musicList, setMusicList] = useState([
    ...
    },
    {
      ...
      data: [],
      type: "featured",
      filterKey: "Trending songs",
    },
    {
      title: "Soul soother",
      data: [],
      type: "featured",
      filterKey: "Soul soother",
    },
    {
      title: "Evergreen melodies",
      data: [],
      type: "featured",
      filterKey: "Evergreen melodies",
    },
  ...
  return (
    <>
      ...
      <div
        className={
          "max-sm:w-screen w-[calc(100%-307px)] h-[calc(100%-64px)] absolute top-[72px] left-0 sm:left-[18.6875rem] bg-[#121212] " +
          (login ? "max-sm:top-0" : "max-sm:top-[56px]")
        }
      >
        {/* Spotify Logo */}
        <div className={"w-full h-6 sm:hidden " + (popSetting ? "hidden" : "")}>
          <Link className="w-[7.875rem] h-6 mt-2 px-6 flex justify-start items-center text-white font-figtree font-medium">
            <BsSpotify className="w-6 h-6 mr-1" />{" "}
        ...
        {login && (
          <Greeting handlePopRender={handlePopRender} popSetting={popSetting} />
        )}

        <div
          className={
            "max-sm:p-0 pt-2 px-6 flex flex-col gap-4 sm:gap-6 " +
            (popSetting ? "hidden" : "")
          }
        >
          {login && (
              ...
              <section className="mb-4 font-figtree">
                ...
                <div className="max-sm:mt-6 max-sm:pl-2 max-sm:mb-3 sm:mb-2">
                  <div className="h-[1.875rem] sm:mb-4 flex justify-between">
                    <Link
                      to={"/more-artist"}
                      className="max-w-[80.5938rem] h-[1.875rem] cursor-pointer "
                    >
                      <h2 className="max-w-fit h-[1.4812rem] text-white font-bold text-2xl hover:underline">
                        Popular artists
                      </h2>
                    </Link>
                    <Link
                      to={"/more-artist"}
                      className="hidden sm:block ml-2 w-[3.4375rem] h-[1.875rem]"
                    >
                      <span className="w-[3.4375rem] h-[1.875rem] font-bold text-sm hover:underline cursor-pointer text-[#A7A7A7]">
                        Show all
                      </span>
                    </Link>
                  </div>
        ...
        {login && (
          <div
            className={
              "bg-black z-10 w-full h-full px-10 " +
              (popSetting ? "" : "hidden")
            }
          >
            {/* Profile */}
...
function Greeting({ handlePopRender, popSetting }) {
  ...
  return (
    <div
      className={
        "sm:hidden h-[50px]  px-6 flex items-center justify-between " +
        (popSetting ? "bg-black" : "")
      }
    >
      <h1 className="text-2xl font-bold hover:underline cursor-pointer font-figtree text-white">
        {popSetting ? "" : greeting}
      </h1>
      <button
        className="sm:hidden w-12 h-12 p-3 flex justify-center items-center"
        onClick={handlePopRender}
      >
        {popSetting ? (
    ...
    <section className="sm:mb-4 font-figtree">
      ...
      <div className=" max-sm:pl-2  sm:mb-2">
        <div className=" h-[2.875rem] sm:mb-4 flex justify-between items-center text-white">
          <div>
            <Link to={path}>
              <span className="text-2xl font-bold font-figtree hover:underline cursor-pointer">
                {musicList.title}
              </span>
            </Link>
          </div>
          <Link
            to={path}
            className="hidden sm:block text-sm font-figtree hover:underline"
          >
            Show all
          </Link>
        </div>
      ...
      <div className="max-sm:h-[13.75rem] sm:h-[calc-[100%-46px]] w-full max-sm:overflow-x-scroll no-scrollbar max-sm:overflow-y-hidden max-sm:flex sm:gap-6 md:flex sm:flex-wrap gap-y-3 md:gap-y-6">
        {musicList.data &&
          musicList.data.map((each) => {
            return <MusicCard key={each._id} musicObj={each} />;
          })}
      </div>
    </section>
  );
