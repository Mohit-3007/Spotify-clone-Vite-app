...
export default function MusicCard({ musicObj, DataFromMusicCard, play, stop }) {
  ...
  return (
    <>
      <Link
        ...
        to={"/songtrack"}
        state={{musicObj: musicObj}}
      >
        <div className="w-[10.8125rem] h-[10.8125rem] mb-4">
          <img src={musicObj.thumbnail} />
        </div>
        {showPlayer && (
          <div className={"w-12 h-12 bg-[#1ED760] rounded-[50%] absolute top-[141px] right-4 " + (showPlayer? "-translate-y-3" : "-translate-y-0")}>
            <button className="w-12 h-12 flex justify-center items-center" onClick={sendDataToParent}>
              <span>
                {" "}
