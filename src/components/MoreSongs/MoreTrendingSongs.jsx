...
export default function MoreTrendingSongs() {
    ...
    async function fetchData() {
      try{
        let resp = await fetch(
            `https://academics.newtonschool.co/api/v1/music/song?page=${page}&limit=24&filter={"featured": "Trending songs"}`,
            {
                headers: headers,
            }
        );
        ...
        console.log("More Trending Songs page ",result);
        setSongData(result);
      }
      catch(error){
